...
VOID Barrage::setBoundingBox(const BoundBoxVec* vBox)
{
	_vBoundBox = *vBox;
}

VOID Barrage::setObjectPool_number(UINT uNumber)
{
	_uPool_number = uNumber;
}

BoundBoxVec& Barrage::getBoundingBox(VOID)
{
...
VOID Barrage::setFire(Character* _Enemy2, Character* Player)
{
	Character* pChar = NULL;
	pChar = _pPool->New(POOL_EnemyBullet);
	pChar->EventReceiver(this);

	if (_Enemy2->getPosition() == Player->getPosition())
	{
		pChar->_pPool->Delete(pChar);
	}
}
