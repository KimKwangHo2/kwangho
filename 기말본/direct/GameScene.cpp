...
VOID GameScene::initEnemy(VOID)
{
	...
	EnemyImage->Source.left	= 0;
	EnemyImage->Source.top	= 0;
	EnemyImage->Source.right = 100;
	EnemyImage->Source.bottom = 100;
	EnemyImage->Time			= 1.0f;


	EnemyImage->Position.x	= -60.50f;
	...
	// 이미지 부르기
	D3DXCreateTextureFromFileExW( _pd3dDevice, L"enemy.jpg", 
		D3DX_DEFAULT_NONPOW2 , D3DX_DEFAULT_NONPOW2 ,
		1, NULL, D3DFMT_UNKNOWN, D3DPOOL_MANAGED,
		D3DX_FILTER_NONE, D3DX_FILTER_NONE, NULL, NULL, NULL, &EnemyImage->Texture); 
...
VOID GameScene::initPlayer(VOID)
{
	...
	*pPlayerImage = PlayerImage;
	...
	D3DXCreateTextureFromFileExW(_pd3dDevice, L"player.png",
		...
		1, NULL, D3DFMT_UNKNOWN, D3DPOOL_MANAGED,
		D3DX_FILTER_NONE, D3DX_FILTER_NONE, NULL, NULL, NULL, &PlayerImage.Texture);
	_Player_texture[3] = PlayerImage.Texture;
...
VOID GameScene::updateKey(VOID)
{
	...
	{
		_Player->moveDown();
		_Player->selAnimation(PLAYER_DOWN);
	}
...
VOID GameScene::Collision(UINT uMsg, Character* pPlayer, Character* pTarget)
{
	const BoundBoxVec& vPlayer = pPlayer->getBoundingBox();
	const BoundBoxVec& vTarget = pTarget->getBoundingBox();


	for (UINT i=0; i<vPlayer.size(); ++i)
	{
		for (UINT k=0; k<vTarget.size(); ++k)
		{
			if ( vTarget[k]->LT.x < vPlayer[i]->RB.x
				&&vPlayer[i]->LT.x < vTarget[k]->RB.x
				&&vTarget[k]->LT.y < vPlayer[i]->RB.y
				&&vPlayer[i]->LT.y < vTarget[k]->RB.y)
			{

				// 충돌메시지 발생
				Event(uMsg, (WPARAM)pPlayer, (LPARAM)pTarget);
				return;
			}
		}
	}
...
VOID GameScene::OnBarrage(UINT uNum)
{
	...
	case 0:
		for (UINT i = 0; i<4; ++i)
		{
			...
			pBarrage->setFire(_Enemy2, _Player);
		}
